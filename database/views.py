...
class AdminTokenObtainPairView(TokenObtainPairView):
    ...
    def set_cookie(self, response, key, value, days_expire=7, secure=True, samesite='None'):
        domain = 'tlga-backend-cf51a0e64d84.herokuapp.com'
        if secure:
            response.set_cookie(key, value, max_age=days_expire * 24 * 60 * 60, secure=True, samesite=samesite, path='/', domain=domain)
        else:
            response.set_cookie(key, value, max_age=days_expire * 24 * 60 * 60, samesite=samesite, path='/', domain=domain)
...
class CustomTokenObtainPairView(TokenObtainPairView):
    ...
    def set_cookie(self, response, key, value, days_expire=7, secure=True, httponly=False, samesite='None'):
        domain = 'tlga-backend-cf51a0e64d84.herokuapp.com'
        if secure:
            response.set_cookie(key, value, max_age=days_expire * 24 * 60 * 60, secure=True, httponly=httponly, samesite=samesite, path='/', domain=domain)
        else:
            response.set_cookie(key, value, max_age=days_expire * 24 * 60 * 60, httponly=httponly, samesite=samesite, path='/', domain=domain)
...
def teacher_profile(request):
    try:
        ...
        return Response("An error occurred while fetching the teacher profile.", status=status.HTTP_500_INTERNAL_SERVER_ERROR)
